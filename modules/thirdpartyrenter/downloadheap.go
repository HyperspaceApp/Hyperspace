...
func (r *ThirdpartyRenter) managedAcquireMemoryForDownloadChunk(udc *unfinishedDownloadChunk) bool {
...
// thread-safe way.
func (r *ThirdpartyRenter) managedAddChunkToDownloadHeap(udc *unfinishedDownloadChunk) {
...
// waiting.
func (r *ThirdpartyRenter) managedBlockUntilOnline() bool {
	// TODO: online check
	// for !r.g.Online() {
	// 	select {
	// 	case <-r.tg.StopChan():
...
// all of the workers.
func (r *ThirdpartyRenter) managedDistributeDownloadChunkToWorkers(udc *unfinishedDownloadChunk) {
...
func (r *ThirdpartyRenter) managedNextDownloadChunk() *unfinishedDownloadChunk {
	r.downloadHeapMu.Lock()
...
// downloads.
func (r *ThirdpartyRenter) threadedDownloadLoop() {
	err := r.tg.Add()
	if err != nil {
		return
	}
...
LOOP:
	for {
		...
		for {
			...
			if time.Now().After(workerUpdateTime.Add(workerPoolUpdateTimeout)) { // TODO: !r.g.Online() ||
