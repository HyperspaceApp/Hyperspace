...
// returned.
func (r *ThirdpartyRenter) managedDownloadLogicalChunkData(chunk *unfinishedUploadChunk) error {
...
// light as possible.
func (r *ThirdpartyRenter) managedFetchLogicalChunkData(chunk *unfinishedUploadChunk) error {
...
func (r *ThirdpartyRenter) managedCleanUpUploadChunk(uc *unfinishedUploadChunk) {
	uc.mu.Lock()
	piecesAvailable := 0
	...
	if chunkComplete && !released {
		...
		if err != nil {
			r.log.Debugf("WARN: file not closed after chunk upload complete: %v %v", uc.fileEntry.HyperspacePath(), err)
		}
