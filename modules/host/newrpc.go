...
func (h *Host) managedRPCLoopFormContract(s *rpcSession) error {
	...
	txnBuilder, newOutputs, newInputs, err := h.managedAddCollateral(settings, txnSet)
	if err != nil {
		s.writeError(err)
		return err
	}
...
func (h *Host) managedRPCLoopRenewContract(s *rpcSession) error {
	...
	txnBuilder, newOutputs, newInputs, err := h.managedAddRenewCollateral(s.so, settings, req.Transactions)
	if err != nil {
		s.writeError(err)
