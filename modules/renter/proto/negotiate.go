...
func verifyRecentRevision(conn net.Conn, contract *SafeContract, hostVersion string, signFunc func(challenge crypto.Hash) (crypto.Signature, error)) error {
	// send contract ID
	...
	// }
	// sign and return
	sig, err := signFunc(challenge)
	if err != nil {
		return errors.New("couldn't sign challenge: " + err.Error())
	}
...
func negotiateRevision(conn net.Conn, rev types.FileContractRevision, signFunc func(crypto.Hash, types.Transaction) (crypto.Signature, error)) (types.Transaction, error) {
	...
	encodedSig, err := signFunc(signedTxn.SigHash(0), signedTxn)
	if err != nil {
