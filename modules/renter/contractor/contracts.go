...
func (c *Contractor) threadedContractMaintenance() {
	...
	for _, contract := range c.staticContracts.ViewAll() {
		...
		if blockHeight+allowance.RenewWindow >= contract.EndHeight {
			...
			// renewal.
			oldContractSpent := contract.TotalCost.Sub(contract.ContractFee).Sub(contract.TxnFee).Sub(contract.RenterFunds)
			...
			estimatedFees := host.ContractPrice.Add(maxTxnFee)
