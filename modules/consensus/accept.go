	...
	setErr := cs.db.Update(func(tx *bolt.Tx) error {
		for i := 0; i < len(blocks); i++ {
			...
			if err == nil {
				...
				for _, b := range changeEntry.AppliedBlockIDs {
					applied = append(applied, b.String()[:6])
				}
				for _, b := range changeEntry.RevertedBlockIDs {
			...
			if len(changeEntry.AppliedBlockIDs) < len(changeEntry.RevertedBlockIDs) {
