...
func assembleServerTester(key crypto.TwofishKey, testdir string) (*serverTester, error) {
	...
	srv, err := NewServer("localhost:0", "Hyperspace-Agent", "", cs, nil, g, h, m, r, tp, w, nil, nil, nil)
	if err != nil {
		return nil, err
	}
...
func assembleAuthenticatedServerTester(requiredPassword string, key crypto.TwofishKey, testdir string) (*serverTester, error) {
	...
	srv, err := NewServer("localhost:0", "Hyperspace-Agent", requiredPassword, cs, nil, g, h, m, r, tp, w, mp, nil, idx)
	if err != nil {
		return nil, err
	}
