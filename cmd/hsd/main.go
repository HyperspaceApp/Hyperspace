...
// The Config struct contains all configurable variables for hsd. It is
...
independent components of Hyperspace. This flag should only be used by developers or
...
func main() {
	...
	root := &cobra.Command{
		Use:   os.Args[0],
		Short: "Hyperspace Daemon v" + build.Version,
		Long:  "Hyperspace Daemon v" + build.Version,
		Run:   startDaemonCmd,
	}
	...
	root.AddCommand(&cobra.Command{
		...
		Long:  "Print version information about the Hyperspace Daemon",
		Run:   versionCmd,
	})
	...
	root.Flags().StringVarP(&globalConfig.Siad.HostAddr, "host-addr", "", config.DefaultHostAddr, "which port the host listens on")
	root.Flags().StringVarP(&globalConfig.Siad.ProfileDir, "profile-directory", "", "profiles", "location of the profiling directory")
	root.Flags().StringVarP(&globalConfig.Siad.APIaddr, "api-addr", "", config.DefaultAPIAddr, "which host:port the API server listens on")
